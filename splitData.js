...
for (const blockName of courseBlocks) {
  // Regex to match from `export const BLOCK_NAME...` to `];` safely.
  // We use a manual brace counting strategy if regex is too complex, but blocks end cleanly with `];` at column 0 or 1.
  ...
  // Also grab the trailing ';' if there is one
  if (content[currentIndex] === ';') {
    blockContent += ';';
  }

  // Find which imports this block needs
  ...
  newTutorialDataImports.push(`import { ${blockName} } from './${lowerName}Data';`);
}
...
finalTutorialData += newTutorialDataImports.join('\n') + '\n\n';
