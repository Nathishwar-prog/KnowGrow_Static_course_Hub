...
import { JS_QUIZ_DATA } from './quizData';
...
const JsQuiz: React.FC = () => {
    ...
    const currentQuestion = JS_QUIZ_DATA[quizState.currentQuestionIndex];
    ...
    const handleNextQuestion = () => {
        if (quizState.currentQuestionIndex < JS_QUIZ_DATA.length - 1) {
            setQuizState({
                ...quizState,
    ...
    const getScoreMessage = (score: number) => {
        const percentage = (score / JS_QUIZ_DATA.length) * 100;
        if (percentage === 100) return "Perfect Score! You're a JavaScript master!";
        if (percentage >= 80) return "Excellent work! You really know your JavaScript.";
    ...
    if (quizState.showResult) {
        return (
            <div className="text-center p-8 bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow-lg">
                <h2 className="text-3xl font-bold mb-4">Quiz Completed!</h2>
                <p className="text-5xl font-bold text-indigo-500 my-4">
                    {quizState.score} / {JS_QUIZ_DATA.length}
                </p>
    ...
    if (!quizState.started) {
        return (
            <div className="text-center p-8 bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow-lg">
                <h2 className="text-4xl font-bold mb-4">JavaScript Quiz</h2>
                <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">Test your knowledge of JavaScript with these {JS_QUIZ_DATA.length} questions.</p>
                <button
    ...
    return (
        <div className="p-4 sm:p-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow-lg">
            <div className="mb-4">
                <p className="text-sm text-gray-500 dark:text-gray-400">Question {quizState.currentQuestionIndex + 1} of {JS_QUIZ_DATA.length}</p>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 mt-2">
                    <div className="bg-indigo-600 h-2.5 rounded-full" style={{ width: `${((quizState.currentQuestionIndex + 1) / JS_QUIZ_DATA.length) * 100}%` }}></div>
                </div>
            </div>
            ...
            <div className="mt-8 text-right">
                {quizState.answerSubmitted ? (
                    ...
                    >
                       {quizState.currentQuestionIndex < JS_QUIZ_DATA.length - 1 ? 'Next Question' : 'Show Results'} <i className="fa-solid fa-arrow-right ml-2"></i>
                    </button>
                ) : (
                    <button
