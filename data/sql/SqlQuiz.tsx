...
import { SQL_QUIZ_DATA } from './quizData';
...
const SqlQuiz: React.FC = () => {
    ...
    const currentQuestion = SQL_QUIZ_DATA[quizState.currentQuestionIndex];
    ...
    const handleNextQuestion = () => {
        if (quizState.currentQuestionIndex < SQL_QUIZ_DATA.length - 1) {
            setQuizState({
                ...quizState,
    ...
    const getScoreMessage = (score: number) => {
        const percentage = (score / SQL_QUIZ_DATA.length) * 100;
        if (percentage === 100) return "Perfect Score! You're an SQL master!";
        if (percentage >= 80) return "Excellent work! You really know your SQL.";
        ...
        return (
            <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
                ...
                <p className="text-5xl font-bold text-blue-500 my-4">
                    {quizState.score} / {SQL_QUIZ_DATA.length}
                </p>
                <p className="text-xl text-gray-600 dark:text-gray-300 mb-8">{getScoreMessage(quizState.score)}</p>
                <button
                    onClick={handleRestartQuiz}
                    className="bg-blue-600 text-white font-bold py-3 px-8 rounded-full hover:bg-blue-700 transition-colors text-lg"
                >
                    Try Again
                </button>
            </div>
    ...
    if (!quizState.started) {
        return (
            <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
                <h2 className="text-4xl font-bold mb-4">SQL Quiz</h2>
                <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">Test your knowledge of SQL with these {SQL_QUIZ_DATA.length} questions from beginner to advanced!</p>
                <button
    ...
    return (
        <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
            <div className="mb-4">
                <p className="text-sm text-gray-500 dark:text-gray-400">Question {quizState.currentQuestionIndex + 1} of {SQL_QUIZ_DATA.length}</p>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 mt-2">
                    <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${((quizState.currentQuestionIndex + 1) / SQL_QUIZ_DATA.length) * 100}%` }}></div>
                </div>
            </div>
            ...
            {quizState.answerSubmitted && (
                 <div className="mt-6 p-4 rounded-md bg-gray-100 dark:bg-gray-700/50 border-l-4 border-gray-400">
                     ...
                     <div className="whitespace-pre-wrap font-mono text-sm mt-2 text-gray-600 dark:text-gray-300">
            ...
            <div className="mt-8 text-right">
                {quizState.answerSubmitted ? (
                    ...
                    >
                       {quizState.currentQuestionIndex < SQL_QUIZ_DATA.length - 1 ? 'Next Question' : 'Show Results'} <i className="fa-solid fa-arrow-right ml-2"></i>
                    </button>
                ) : (
                    <button
