...
const ColorBox: React.FC<{ name: string, hex: string }> = ({ name, hex }) => (
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm text-center">
        <div style={{ backgroundColor: hex }} className="h-24 rounded-t-lg"></div>
...
const HtmlColorsRef: React.FC = () => (
    <>
        <p className="text-lg leading-relaxed">HTML supports 140 standard color names. This page provides a reference to all of them, along with their hexadecimal values.</p>
        <p className="text-lg leading-relaxed mt-4">You can use these names directly in your CSS or `style` attributes, for example: `style="color:Tomato;"`.</p>
        
        <hr className="my-8 border-gray-200 dark:border-gray-600" />
        
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {colors.map(color => <ColorBox key={color.name} name={color.name} hex={color.hex} />)}
        </div>
    </>
);

export default HtmlColorsRef;